...
function Row({ node, depth }: { node: ResultTreeNode; depth: number }) {
  return (
    <div>
      <div
        className="flex items-center gap-2 rounded py-1 pr-2 text-sm leading-caption hover:bg-neutral-50"
        style={{ paddingLeft: `${depth * 14 + 4}px` }}
      >
        <span aria-hidden className="shrink-0 text-neutral-400">📁</span>
        <span className="truncate">{node.title}</span>
        {node.isNew && (
          <span className="shrink-0 rounded bg-green-100 px-1 text-2xs text-green-700">{t('resultTreeNew')}</span>
        )}
