...
describe('parseImportFile 文件级校验', () => {
  ...
  it('format 不认识时报错', () => {
    ...
    expect(parseImportFile(text)).toEqual({
      ok: false, error: '这不是 TidyMark 导出的文件，或者版本不支持。',
    })
  })
  ...
  it('kind 为 tree 但 roots 不是数组时报错', () => {
    ...
    expect(parseImportFile(text)).toEqual({ ok: false, error: '文件结构损坏。' })
  })
...
describe('buildImportPreview 拦截不安全链接', () => {
  it('拦下 javascript: 并记进 blocked', () => {
    ...
    expect(preview.blocked).toEqual([
      { name: 'Gmail', url: 'javascript:alert(1)', reason: '不安全的链接类型（javascript:）' },
    ])
  })
  ...
  it('拦下 data:', () => {
    ...
    const preview = buildImportPreview(doc, tree, AT)
    expect(preview.nodes).toEqual([])
    expect(preview.blocked[0]!.reason).toBe('不安全的链接类型（data:）')
  })
...
describe('buildImportPreview 重复计数', () => {
  it('数的是已存在于书签库里的条数', () => {
    ...
    expect(buildImportPreview(doc, tree, AT).duplicateCount).toBe(1)
  })
  ...
  it('同一个已有 URL 出现两次算两条，不是一种', () => {
    ...
    expect(buildImportPreview(doc, tree, AT).duplicateCount).toBe(2)
  })

  it('一条都不重复时为 0', () => {
    const doc = parsed(treeDoc([{ name: 'x', url: 'https://brand.new' }]))
    expect(buildImportPreview(doc, tree, AT).duplicateCount).toBe(0)
  })
})
...
describe('buildImportPreview 目标位置', () => {
  it('目标文件夹名用本地日期', () => {
    const doc = parsed(treeDoc([]))
    expect(buildImportPreview(doc, tree, AT).targetName).toBe('导入 2026-08-04')
  })
  ...
  it('barTitle 取自树里的书签栏，不是写死的字符串', () => {
    ...
    const doc = parsed(treeDoc([]))
    expect(buildImportPreview(doc, renamed, AT).barTitle).toBe('Bookmarks Bar')
  })
})
