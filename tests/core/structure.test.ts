...
describe('buildStructureView', () => {
  it('产出两层结构，标题不含编号前缀', () => {
    const view = buildStructureView(makePlan(), EMPTY_EDITS)
  ...
  it('已删除的节点不出现在视图里', () => {
    const view = buildStructureView(makePlan(), { renames: {}, removed: ['tmp:4'] })
    expect(view[1]!.children).toEqual([])
  })

  it('改名反映在视图标题上', () => {
    const view = buildStructureView(makePlan(), { renames: { 'tmp:1': '代码仓库' }, removed: [] })
...
describe('applyStructureEdits 改名', () => {
  it('写回裸名字，不带编号前缀', () => {
    const next = applyStructureEdits(makePlan(), { renames: { 'tmp:1': '代码仓库' }, removed: [] })
...
describe('applyStructureEdits 删除', () => {
  it('删一级主题目录，书签回落到「其他」', () => {
    const next = applyStructureEdits(makePlan(), { renames: {}, removed: ['tmp:3'] })
  ...
  it('删二级子目录，书签回落到父目录而非「其他」', () => {
    const next = applyStructureEdits(makePlan(), { renames: {}, removed: ['tmp:4'] })
    expect(targetOf(next, 'r0')).toBe('tmp:3')
  })

  it('删聚合目录，书签按 primaryTopic 回落到同名主题目录', () => {
    const next = applyStructureEdits(makePlan(), { renames: {}, removed: ['tmp:1'] })
  ...
  it('被删的目录从 candidates 里移除', () => {
    const next = applyStructureEdits(makePlan(), { renames: {}, removed: ['tmp:3', 'tmp:4'] })
  ...
  it('空编辑返回内容等价的 plan', () => {
    const plan = makePlan()
    const next = applyStructureEdits(plan, EMPTY_EDITS)
